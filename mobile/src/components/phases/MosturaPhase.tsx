...
export function MosturaPhase({ phase, session }: MosturaPhaseProps) {
  ...
  function isStepDone(index: number) {
    return !!session.completedSteps[`mostura:${index}`]
  }
  ...
  return (
    <View>
      {phase.steps.map((step, index) => {
        ...
        if (step.type === "timer") {
          const timerId = `mostura-${session.id}`
      ...
      {allDone && (
        <TouchableOpacity
          ...
          onPress={() => setPhase(session.id, "fervura")}
        >
